  ...
  IF v_state.processed_count + v_state.failed_count >= v_state.total_count THEN
    ...
    IF v_state.failed_count > 0 AND v_retry_pass < v_max_retry THEN
      ...
      SET
        ...
        failed_items  = '[]'::jsonb,
        failed_count  = 0,
        status        = 'paused',  -- l'auto-resume du connecteur reprendra
        last_tick_at  = now(),
