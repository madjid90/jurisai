  ...
  try {
    ...
    EdgeRuntime.waitUntil((async () => {
      ...
      try {
          ...
          console.log(`[connector-cdtn-modeles-full] batch ${batchId} fini: status=${fin.status} processed=${fin.processed}/${fin.total} ingested=${ingested} skipped=${skipped} failed=${failed}`);
          ...
          if (fin.status === "paused") {
            ...
            if (cs && su) {
              try {
                await fetch(`${su}/functions/v1/connector-cdtn-modeles-full`, {
                  method: "POST",
                  headers: {
                ...
                console.log(`[connector-cdtn-modeles-full] auto-resume scheduled for batch ${batchId}`);
              } catch (e) {
                console.warn(`[connector-cdtn-modeles-full] auto-resume failed:`, (e as Error).message);
              }
            }
          }
